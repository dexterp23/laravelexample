...
class PagesRepository
{
	...
	public function getDef( int $ID_users, String $type ): Collection {
		return $this->model
							->where( $type, 1 )
							->where( 'ID_users', $ID_users )
							->get();
	...
	public function add( Array $attributtes ): Pages {
	...
	public function updatedef( String $attributt, Int $id ): Bool {
		$ID_users = Auth::id();
        return $this->model
							->where( 'ID_users', $ID_users )
							->where( 'id', $id )
                           	->update( array($attributt => 1) );
    }
	
	public function updatedefReset( Array $attributtes ): Bool {
		$ID_users = Auth::id();
        return $this->model
							->where( 'ID_users', $ID_users )
                           	->update( $attributtes );
