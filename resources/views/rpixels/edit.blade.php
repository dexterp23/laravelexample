...
@section('content')
	...
	<div class="header">
        <h1 class="header-title">
            @if(!empty($data[0]->name)){{ $data[0]->name }}@else{{ 'Retargeting Pixel' }}@endif
        </h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}"><i class="fas fa-fw fa-home"></i></a></li>
                <li class="breadcrumb-item"><a href="{{ route('rpixels.list') }}">Retargeting Pixels</a></li>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
					...
					<form action="{{ route('rpixels.update') }}" method="post" name="form" id="form" class="">
                    	@method('POST')
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea rows="5" name="code" class="form-control">@if(!empty($data[0]->code)){{ $data[0]->code }}@else{{ '' }}@endif</textarea>
                            </div>
                        </div>
                        ...
                        <div class="form-actions">
							...
							@if(!empty($data[0]))
                                <a href="{{ route('rpixels.add') }}" class="btn btn-success float-right ml-1">Add New</a>
                                ...
                                <script>
                                    function rusure(){
                                        ...
                                        if (question !="0"){
                                            top.location = "{{ route('rpixels.delete', $data[0]->id) }}"
                                        }
                                    }
                                </script>
                            @endif
