                ...
                <div class="card-header">
                    <form id="form" name="form" method="get">
                        ...
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <a href="{{ route('domains.add') }}" class="btn btn-success float-left">Add New</a>
                                
                                    <input type="submit" value="Search" class="btn btn-primary float-right ml-1">
                                    <a href="{{ route('domains.list') }}" class="btn btn-warning float-right ml-1">Reset</a>
                                </div>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-striped table-bordered table-highlight-head">
                                ...
                                @foreach ($data as $data_v)
                                    <tr>
                                        <td><a href="{{ route('domains.edit', $data_v->id) }}">{{ $data_v->name }}</a></td>
                                        <td><a href="{{ $data_v->url }}" target="_blank">{{ $data_v->url }} <i class="fas fa-fw fa-external-link-alt"></i></a></td>
                                        <td class="table_center"><a href="{{ route('domains.edit', $data_v->id) }}" class="btn btn-sm btn-primary">Edit</a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
